...
Между "детской" и "кухней" — три мини-игры подряд: велогонка (вид
сверху, три полосы, уклоняйся от камней), теннис (лови мяч ракеткой)
и футбол (выбери сторону и пробей мимо вратаря). Нигде нет проигрыша —
везде либо таймер, либо ограничение по попыткам.
...
make test       # юнит-тесты логики (нужен обычный gcc, эмулятор не нужен)
```
...
- Юнит-тесты (`tests/`, запуск `make test`): game.c компилируется обычным
  gcc с заглушками вместо железа NES — транслит, коллизии, звук, музыка
  и логика мини-игр проверяются на компьютере за доли секунды.
...
- День 17 — ещё декора: телевизор в гостиной, мобиль в детской,
  холодильник на кухне, фонарь на улице
- День 18 — три мини-игры между уровнями: велогонка, теннис, футбол.
  У них свой набор графики — задействована вторая половина видеопамяти
  катриджа, которая раньше просто пустовала
